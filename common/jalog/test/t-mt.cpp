...
#define clog(...) AC_JALOG_SCOPE(common, Info, __VA_ARGS__)
#define tlog(...) AC_JALOG_SCOPE(st, Info, __VA_ARGS__)
#define tlog(...) AC_JALOG_SCOPE(st, Info, __VA_ARGS__)

const TestSink::EntryCopy* findByText(const std::vector<TestSink::EntryCopy>& es, std::string_view text) {
    return astl::pfind_if(es, [text](const TestSink::EntryCopy& e) { return e.text == text; });
}
...
TEST_CASE("mt")
{
...

    {
        ...
        ac::jalog::Scope common(jl, "common", 1, 2);
        ...
        std::thread t1([&]() {
            clog("in t", 1);
            ac::jalog::Scope st(jl, "st1", 10, 20);
            tlog(1);
            tlog(2);
            clog("mix", 1);
            tlog(3);
        ...
        std::thread t2([&]() {
            clog("in t", 2);
            ac::jalog::Scope st(jl, "st2", 50, 80);
            tlog(5);
            tlog(6);
            clog("mix", 2);
            tlog(7);
