...
TEST_CASE("expandPath") {
    ...
    {
        auto e = ac::local::fs::expandPath("~/foo");
    ...
    auto test = [](const std::string& path, std::string_view expected) {
        auto expanded = ac::local::fs::expandPath(path);
        CHECK(expanded == expected);
        auto inplace = path;
        ac::local::fs::expandPathInPlace(inplace);
    ...
    CHECK_THROWS_WITH_AS(ac::local::fs::expandPath("$nope42/xxx"), "Environment variable nope42 not set", std::runtime_error);
}
...
TEST_CASE("stat") {
    auto s = ac::local::fs::basicStat(binDir);
    CHECK(s.exists());
    CHECK(s.dir());
    CHECK(!s.file());
    CHECK(s.size == 0);

    s = ac::local::fs::basicStat(bin1);
    CHECK(s.exists());
    CHECK(s.file());
    CHECK(!s.dir());
    CHECK(s.size == TA_BINARY_FILE_SIZE);

    s = ac::local::fs::basicStat(bin2);
    CHECK(s.exists());
    CHECK(s.file());
    CHECK(!s.dir());
    CHECK(s.size == TA_ANOTHER_BINARY_FILE_SIZE);

    s = ac::local::fs::basicStat(nope);
    CHECK(!s.exists());
    CHECK(!s.file());
    CHECK(!s.dir());
    CHECK(s.size == 0);
